...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 1. Embed the source sentences
        X = self.model_embeddings.source(source_padded)

        # 2. Pack the sequence, run through the encoder, unpack the sequence
        X_packed = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_packed)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)

        # 3. Permute the shape of the enc_hiddens tensor
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # changing dim 0 and 1

        # 4. Prepare decoder's initial state
        # Concatenate the forwards and backwards states and apply the projection layers
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 3. Iterate over each time step using torch.split
        for Y_t in torch.split(Y, 1, dim=0):
            Y_t = torch.squeeze(Y_t, 0)  # Squeeze Y_t into shape (b, e)
            # Concatenate Y_t with o_prev to get Ybar_t
            Ybar_t = torch.cat([Y_t, o_prev], dim=1)

            # Compute the next decoder state and the combined output vector o_t
        ...
        # 4. Convert combined_outputs to a tensor (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 3. Compute the attention scores e_t using batch matrix multiplication
        ...
        # Reshape for bmm, now alpha_t is shape (b, 1, src_len)
        alpha_t = alpha_t.unsqueeze(1)
        a_t = torch.bmm(alpha_t, enc_hiddens).squeeze(
            1)  # Result is shape (b, 2h)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            prev_hyp_ids = top_cand_hyp_pos.div(
                len(self.vocab.tgt), rounding_mode="floor")
            # top_cand_hyp_pos % len(self.vocab.tgt)
            hyp_word_ids = top_cand_hyp_pos - \
                top_cand_hyp_pos.div(
                    len(self.vocab.tgt), rounding_mode="floor") * len(self.vocab.tgt)
